...
def merge_sort_thread(arr):
    ...
    if len(arr) > 1:
        ...
        # Sorting the first half
        left_half_thread = threading.Thread(
            target=merge_sort_thread, args=(L,))
        left_half_thread.start()

        # Sorting the second half
        right_half_thread = threading.Thread(
            target=merge_sort_thread, args=(R,))
        right_half_thread.start()

        threads_list = [left_half_thread, right_half_thread]
