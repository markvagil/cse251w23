...
class Maze():
    ...
    def __init__(self, screen, width, height, bitmap_file, delay=False):
        ...
        self.colors = [[COLOR_BLACK for _ in range(
    ...
    def move(self, row, col, color):
        ...
        if state != OPEN:
            print(
                f'ERROR: You are trying to move on a spot that is a wall or already visited: {row}, {col}, color = {self.colors[row][col]}')
            return

        self.colors[row][col] = color
        pos_x, pos_y = self._calc_screen_pos(row, col)
        self.screen.block(pos_x, pos_y, self.scale_w,
                          self.scale_h, color=color)
        self.screen.update()
        if self.delay:
    ...
    def restore(self, row, col):
        ...
        pos_x, pos_y = self._calc_screen_pos(row, col)
        self.screen.block(pos_x, pos_y, self.scale_w,
                          self.scale_h, color=COLOR_VISITED)
        self.screen.update()
        ...
        for row in range(self.height):
            for col in range(self.width):
                if self._state(row, col) == OPEN:
                    pos_x, pos_y = self._calc_screen_pos(row, col)
                    self.screen.block(pos_x, pos_y, self.scale_w,
                                      self.scale_h, color=COLOR_WHITE)
                else:
                    pos_x, pos_y = self._calc_screen_pos(row, col)
                    self.screen.block(pos_x, pos_y, self.scale_w,
                                      self.scale_h, color=COLOR_BLACK)
        self.screen.update()
