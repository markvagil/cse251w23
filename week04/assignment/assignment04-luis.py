...
class Manufacturer(threading.Thread):
    ...
    def __init__(self, car_count, is_dealer_full: threading.Semaphore, make_more_cars: threading.Semaphore, table_queue, table_lock):
        ...
        self.is_dealer_full = is_dealer_full
        self.make_more_cars = make_more_cars
    ...
    def run(self):
        for i in range(self.car_count):
           ...
           """
            self.is_dealer_full.acquire()

            add_car = Car()

            self.table_lock.acquire()
            self.table_queue.put(add_car)
            self.table_lock.release()

            self.make_more_cars.release()
...
class Dealership(threading.Thread):
    ...
    def __init__(self, queue_stats, is_dealer_full: threading.Semaphore, make_more_cars: threading.Semaphore, table_queue, table_lock):
        threading.Thread.__init__(self)
    ...
    def run(self):
        while True:
            ...
            """
            self.make_more_cars.acquire()
            # self.table_lock.acquire()

            self.stats[self.table_queue.size()-1] += 1
            new_car = self.table_queue.get()
...
def main():
    ...
    is_dealer_full = threading.Semaphore(MAX_QUEUE_SIZE)
    make_more_cars = threading.Semaphore(0)
    # TODO Create queue (ONLY use class QueueTwoFiftyOne)
    table_queue = QueueTwoFiftyOne()
    # TODO Create lock
    ...
    manufacturer = Manufacturer(
        CARS_TO_PRODUCE, is_dealer_full, make_more_cars, table_queue, table_lock)
    # TODO create your one dealership
    dealership = Dealership(queue_stats, is_dealer_full,
                            make_more_cars, table_queue, table_lock)
