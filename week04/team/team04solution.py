...
class Feeder(threading.Thread):
    ...
    def run(self):
        ...
        for _ in range(FOOD_TO_MAKE):
            ...
            new_food = Food()
            print(f"+ Made {new_food}")
            self.table_queue.put(new_food)
...
def main(number_of_eaters, number_of_feeders):
    ...
    if len(table_queue.items) == 0:
        print("Table empty.")
    else:
        print(f"Table not empty. {len(table_queue.items)} food remaining.")
        print(table_queue.items)
